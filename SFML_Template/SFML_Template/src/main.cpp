...
int main() 
{
	...
	while (window.isOpen()) 
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (lives > 0 && gameTime.getElapsedTime().asSeconds() < 120)
		...
		for (int n = 0; n < 9; n++)
			if (frog.frogSprite.getGlobalBounds().intersects(car.cars[n].getGlobalBounds()))
			{
				if (lives > 1)
				{
					frog.kill();		// resets frog position
		...
		if (frog.frogSprite.getPosition().y < 260 && frog.frogSprite.getPosition().y > 90)		// if frog is between water boundaries
		{
			for (int n = 0; n < 9; n++)
			{
				if (frog.frogSprite.getGlobalBounds().intersects(car.logs[n].getGlobalBounds()))		// checks if frog is intersecting a log
				{
					...
					if (event.type != sf::Event::KeyPressed)
						frog.frogSprite.setPosition(car.logs[n].getPosition());		// makes the frogs position match the log until a kep is pressed
				}
			}
		...
		//// ON INTERSECTION OF AREAS BETWEEN BASES ////
		/*if((frog.frogSprite.getPosition().y < 90) && (frog.frogSprite.getPosition().x < 70))
		{
			if (lives > 1)
			{
				frog.kill();		// resets frog position
		...
		if ((frog.frogSprite.getPosition().y < 90) && (frog.frogSprite.getPosition().x < 265) && (frog.frogSprite.getPosition().x > 120))
		{
			if (lives > 1)
			{
				frog.kill();		// resets frog position
		...
		if ((frog.frogSprite.getPosition().y < 90) && (frog.frogSprite.getPosition().x < 460) && (frog.frogSprite.getPosition().x > 315))
		{
			if (lives > 1)
			{
				frog.kill();		// resets frog position
		...
		if ((frog.frogSprite.getPosition().y < 90) && (frog.frogSprite.getPosition().x > 510))
		{
			if (lives > 1)
			{
				frog.kill();		// resets frog position
