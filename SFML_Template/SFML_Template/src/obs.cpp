...
obs::obs()
{
	...
	Lwidth = Slog.getSize().x;
	Lheight = Slog.getSize().y;
	Loff = (Slog.getSize().x * 0.5);
	...
	for (int n = 0; n < 3; n++)
	{
		...
		logs[n].setOrigin((Lwidth / 2), (Lheight / 2));
...
void obs::moveRight()
{
	for (int n = 0; n < 6; n++)
	{
		if (n < 3)
		{
			...
			if (logs[n].getPosition().x > (600 + Loff))
				logs[n].setPosition((0 - Loff), 165);		// resets position if it goes off the edge of the screen
			else
...
void obs::moveLeft()
{
	for (int n = 3; n < 9; n++)
	{
		if (n < 6)
		{
			if (logs[n].getPosition().x < (0 - Loff))
				logs[n].setPosition((600 + Loff), 100);		// resets position if it goes off the edge of the screen
			else
		...
		else
		{
			...
			if (logs[n].getPosition().x < (0 - Loff))
				logs[n].setPosition((600 + Loff), 230);		// resets position if it goes off the edge of the screen
			else
